...
def load_real_data():
    ...
    try:
        print(f"Intentando cargar datos desde S3: {S3_BUCKET_REAL}/{S3_KEY_JSON}")
        
        # Intentar leer el archivo JSON desde S3
        response = s3_client.get_object(Bucket=S3_BUCKET_REAL, Key=S3_KEY_JSON)
        data = json.loads(response['Body'].read().decode('utf-8'))
        
        print(f"JSON cargado exitosamente. Estructura: {list(data.keys())}")
        
        # Verificar si tiene la estructura esperada
        if 'messages' not in data:
            print("Error: El JSON no tiene la estructura esperada (falta 'messages')")
            return pd.DataFrame()
        ...
        if 'Title' in df.columns:
            ...
            print("Advertencia: No se encontró la columna 'Title'")
